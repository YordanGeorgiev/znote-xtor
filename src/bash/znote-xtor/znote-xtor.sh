#!/bin/bash 
# file: src/bash/znote-xtor.sh doc at the eof file
...
doRunActions(){
   ...
   test -z "${actions+1}" && doPrintUsage && doExit 0 
	...
	while read -d ' ' action ; do (
		...
		done < <(find src/bash/znote-xtor/funcs -type f -name '*.sh')
...
doSetVars(){
   ...
   # set -x
	test -z "${znote_xtor_project:-}" && doParseConfFile
	#test -z "${znote_xtor_project:-}" || doSetUndefinedShellVarsFromCnfFile
   # sleep 20
