#!/bin/bash 
# file: src/bash/znote-xtor/test-znote-xtor.sh
      ...
      doLogTestRunEntry 'START'
		while read -r test_file ; do (
            ...
            then
               ...
               $function_name
               doLog "INFO test-znote-xtor loop exit_code: $exit_code" 
               doLogTestRunEntry 'INFO' ' '$(date "+%H:%M:%S")
		...
		done < <(find src/bash/znote-xtor/tests -type f -name '*.sh')

	);
	done < <(cat $product_instance_dir/src/bash/znote-xtor/tests/run-znote-xtor-tests.lst)
...
doLog(){
   ...
   test -t 1 && echo " [$type_of_msg] `date "+%Y.%m.%d-%H:%M:%S"` [znote-xtor][@$host_name] [$$] $msg "
...
doSetVars(){
   ...
   . "$product_instance_dir/src/bash/znote-xtor/funcs/log-test-run-entry.func.sh"
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the znote-xtor
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: src/bash/znote-xtor/test-znote-xtor.sh
