package ZnoteXtor::App::IO::In::RdrJson;
...
use base qw(ZnoteXtor::App::Utils::OO::SetGetable ZnoteXtor::App::Utils::OO::SetGetable);
...
=head1 SYNOPSIS
	...
	doResolves the product version and base dirs , bootstraps cnfig files if needed
		...
		use ZnoteXtor::App::ETL::ZnoteXtor ;
		my $objZnoteXtor = 
			'ZnoteXtor::App::ETL::ZnoteXtor'->new ( \$appConfig ) ; 
=cut 
...
sub doConvertStrToHashRef {
  ...
  my $objTimer = 'ZnoteXtor::App::Utils::Timer'->new();
...
sub doReadIssueFile {
  ...
  my $znote_xtor_project = $ENV{"znote_xtor_project"};
  my $objTimer = 'ZnoteXtor::App::Utils::Timer'->new();
  my ($sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst)
   = $objTimer->GetTimeUnits();
  ...
  my $nice_date  = "$year" . '-' . "$mon" . '-' . $mday;
    ...
    $issues_file
      = $ENV{'mix_data_dir'}
      . '/issues'
      . "/$year/$nice_month/$nice_date/$znote_xtor_project"
      . '.' .$table_file_name . '.'
      . "$nice_date" . '.txt' ; 
...
sub doInitialize {
  ...
  $objFileHandler
    = 'ZnoteXtor::App::Utils::IO::FileHandler'->new(\$appConfig);
